...
import { signinValidator, type SigninForm } from "~~/forms/signin";

definePageMeta({
  layout: false,
});

const dataObject = ref<Partial<SigninForm>>({});
...
const errors = computed(() => {
  const rawErrors = signinValidator(dataObject.value);
...
function submit() {
  ...
  loading.value = true;
  $appFetch("/api/v1/auth/signin", { body: dataObject.value, method: "POST" })
    .then((e) => {
      console.log(e);
      router.push("/");
    })
    ...
    <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-[480px]">
      <div class="bg-white px-6 py-12 shadow-sm sm:rounded-lg sm:px-12">
        <form @submit.prevent="submit" class="space-y-6">
          ...
          <div>
            <SpinningButton :spinning="loading"> Sign in </SpinningButton>
          </div>
        </form>
