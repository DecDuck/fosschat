...
import { signupValidator, type SignupForm } from "~~/forms/signup";

definePageMeta({
  layout: false,
});

const dataObject = ref<Partial<SignupForm>>({});
const loading = ref(false);
const remoteError = ref<undefined | string>();

const router = useRouter();

const errors = computed(() => {
  const rawErrors = signupValidator(dataObject.value);
  if (!(rawErrors instanceof type.errors)) return {};
  const errors: { [key: string]: string } = {};
  for (const error of rawErrors) {
    const path = error.path.toString() as keyof typeof dataObject.value;
    if (!dataObject.value[path]) continue;
...
function submit() {
  ...
  loading.value = true;
  $appFetch("/api/v1/auth/signup", { body: dataObject.value, method: "POST" })
    .then((e) => {
      console.log(e);
      router.push("/signin");
    })
...
<template>
  ...
  >
    ...
    <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-[480px]">
      <div class="bg-zinc-950 px-6 py-12 shadow-sm sm:rounded-lg sm:px-12">
        <form @submit.prevent="submit" class="space-y-6">
          ...
          <div class="flex items-center justify-center">
            <NuxtTurnstile v-model="dataObject.token" />
          </div>
          ...
          <div>
            <SpinningButton :spinning="loading">
              Create your account
            </SpinningButton>
          </div>
        </form>
        ...
        <div>
          <div class="relative mt-10">
            ...
            <div class="relative flex justify-center text-sm/6 font-medium">
              <span class="bg-zinc-950 px-6 text-zinc-100">Or continue with</span>
            </div>
          </div>
